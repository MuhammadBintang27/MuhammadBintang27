  ...
  >
    <div className="mx-auto grid w-full max-w-6xl items-center gap-12 md:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)] md:gap-16">
      ...
      {/* Left — framed photo with the education pill overlapping the base.
          Outer div owns the entrance (bouncy overshoot settle from a tilt);
          inner div owns an endless gentle bob once it has landed, same
          split used on the Hero photo so the two motions don't fight. */}
      <motion.div
        initial={{ opacity: 0, x: -40, rotate: -8, scale: 0.94 }}
      ...
      >
        <motion.div
          ...
          className="relative overflow-hidden rounded-[2rem] border-[6px] border-[#F0A716] bg-[#EAD2A8] shadow-[10px_12px_0_rgba(36,26,18,0.16)]"
        >
          <img
        ...
        {/* Education pill — star logo + degree, straddles the bottom edge,
            then wiggles gently in place forever once it has popped in. */}
        <motion.div
          initial={{ opacity: 0, x: '-50%', y: 20, scale: 0.7, rotate: -6 }}
          whileInView={{ opacity: 1, x: '-50%', y: 0, scale: 1, rotate: -2 }}
          viewport={{ once: true, amount: 0.3 }}
          transition={{ duration: 0.55, delay: 0.4, ease: [0.34, 1.56, 0.64, 1] }}
          className="absolute -bottom-6 left-1/2 w-[90%]"
        >
          <motion.div
            animate={{ rotate: [-2, 2, -2] }}
            transition={{ duration: 4.2, repeat: Infinity, ease: 'easeInOut', delay: 1.1 }}
            className="flex items-center gap-3 rounded-full bg-[#E5301E] px-4 py-3 shadow-[5px_5px_0_rgba(36,26,18,0.18)]"
          >
            <img
      ...
      {/* Right — greeting, bio, CTAs. Staggered pop-in: heading bounces in
          first, bio fades up behind it, buttons pop in one after another. */}
      <motion.div
        initial="hidden"
        whileInView="show"
        viewport={{ once: true, amount: 0.3 }}
      ...
      >
        <motion.h2
          variants={{
            ...
            show: { opacity: 1, y: 0, scale: 1, rotate: 0, transition: { duration: 0.55, ease: [0.34, 1.56, 0.64, 1] } },
          }}
          className="font-mouse text-[clamp(3rem,8vw,6rem)] uppercase leading-[0.95] text-[#E5301E] [-webkit-text-stroke:2px_#FFFDF8] [paint-order:stroke_fill] drop-shadow-[4px_4px_0_rgba(36,26,18,0.14)]"
        >
          Hola, I&apos;m Bintang
        </motion.h2>
        ...
        <motion.p
          ...
          className="mx-auto mt-6 max-w-xl font-mouse text-lg leading-snug tracking-wide text-[#241A12] sm:text-xl md:mx-0"
        >
          {PROFILE.bio}
        </motion.p>
        ...
        <div className="mt-8 flex flex-row flex-wrap items-center justify-center gap-3 sm:gap-4 md:justify-start">
          <motion.button
            ...
            className="group inline-flex items-center gap-3 rounded-full bg-[#E5301E] px-7 py-3 text-base font-bold text-[#FFFDF8] shadow-[4px_4px_0_rgba(36,26,18,0.18)] transition-colors hover:bg-[#C22112] active:translate-y-0.5"
          >
          ...
          <motion.button
            ...
            className="group inline-flex items-center gap-3 rounded-full bg-[#241A12] px-7 py-3 text-base font-bold text-[#FFFDF8] shadow-[4px_4px_0_rgba(36,26,18,0.18)] transition-colors hover:bg-[#3a2a1c] active:translate-y-0.5"
          >
            Get in Touch
