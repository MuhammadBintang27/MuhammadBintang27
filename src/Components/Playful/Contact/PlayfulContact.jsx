...
const PlayfulContact = () => (
  ...
  >
    ...
    {/* Splatter — cream droplets flung across the pour + a couple of red flecks
        thrown up above the crown, selling the "just spilled" look. A few
        drift with a lazy infinite bob so the pour never sits dead still. */}
    <div className="pointer-events-none absolute inset-0 z-[5]" aria-hidden="true">
      ...
      <motion.span
        ...
        className="absolute left-[14%] top-[34%] h-3 w-3 rounded-full bg-[#F2E1C4] sm:h-4 sm:w-4"
      />
      <motion.span
        animate={{ y: [0, -14, 0] }}
        transition={{ duration: 4.4, repeat: Infinity, ease: 'easeInOut', delay: 1 }}
        className="absolute right-[11%] top-[40%] h-5 w-5 rounded-full bg-[#F2E1C4] sm:h-6 sm:w-6"
      />
      ...
      <motion.span
        ...
        className="absolute right-[8%] bottom-[24%] h-4 w-4 rounded-full bg-[#F0A716] sm:h-5 sm:w-5"
      />
      <span className="absolute left-[16%] bottom-[22%] h-3 w-3 rounded-full bg-[#F0A716]" />
    </div>

    {/* Content — sits straight on the poured paint. Staggered pop-in so the
        eyebrow, title, copy and CTA land one beat after another instead of
    ...
    >
      <motion.p
        ...
        className="mb-4 font-modak text-sm uppercase tracking-[0.28em] text-[#FFD23F]"
      >
        Get in Touch
      </motion.p>
      ...
      <motion.h2
        variants={{
          hidden: { opacity: 0, y: 34, scale: 0.88, rotate: -2 },
          show: { opacity: 1, y: 0, scale: 1, rotate: 0, transition: { duration: 0.6, ease: [0.34, 1.56, 0.64, 1] } },
        }}
        className="font-mouse text-[clamp(2.8rem,10vw,6.5rem)] uppercase leading-[0.9] tracking-tight text-[#FFFDF8] [-webkit-text-stroke:2px_#241A12] [paint-order:stroke_fill] drop-shadow-[5px_6px_0_rgba(36,26,18,0.22)]"
      >
        Let&apos;s Talk!
      </motion.h2>
      ...
      <motion.p
        ...
        className="mx-auto mt-6 max-w-[46ch] font-mouse text-lg leading-snug tracking-wide text-[#FFFDF8]/90 sm:text-xl"
      >
      ...
      <motion.a
        ...
        className="group mt-9 inline-flex items-center gap-3 rounded-full bg-[#241A12] px-8 py-3.5 text-base font-bold text-[#FFFDF8] shadow-[5px_5px_0_rgba(36,26,18,0.28)] transition-colors hover:bg-[#3a2a1c] active:translate-y-0.5"
      >
        Say Hello
